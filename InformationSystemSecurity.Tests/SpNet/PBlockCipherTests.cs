...
public class PBlockCipherTests
{
    ...
    public void Encrypt_ReturnsCorrectEncryptedString(string @in, int round, string expected)
    {
    ...
    [InlineData("ЗОЛОТАЯ_СЕРЕДИНА", 2, "ЛДОИНЗСЯАЕТРЕ_АО")]
    [InlineData("ЗОЛОТАЯ_СЕРЕДИНА", 4, "ОЩКЬРСВПИЗАТВЬЛЧ")]
    public void Decrypt_ReturnsOriginalString(string expected, int round, string @in)
    {
